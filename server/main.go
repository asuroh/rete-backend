...
import (
	...
	"fmt"
	"os"
	"retel-backend/pkg/env"
	"retel-backend/pkg/interfacepkg"
...
func main() {
	...
	// Sql DB connection
	db, err := sql.Open("mysql", envConfig["DATABASE_USER"]+":@tcp("+envConfig["DATABASE_HOST"]+":"+envConfig["DATABASE_PORT"]+")/"+envConfig["DATABASE_DB"])
	if err != nil {
